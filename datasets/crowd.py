...
class Crowd_UCF_CC_50(Base):
    ...
    def __getitem__(self, item):
        ...
        gd_path = os.path.join(
            self.root_path, "ground_truth", "{}_ann.mat".format(name)
        )
        img = Image.open(img_path).convert("RGB")
...
class CustomDataset(Base):
    """
    Class yang memungkinkan training untuk custom dataset. Struktur folder:
    root_dataset_path:
        -> train/
            -> gambar1.jpg
            -> gambar1.txt
        ...
        -> test/
            -> gambar_test1.jpg
            -> gambar_test1.txt (bisa kosong untuk test)
        -> train.list
        -> valid.list
        -> test.list

    Isi file list (CSV dengan space sebagai separator):
        img_xx_path label_xx_path
        img_xx1_path label_xx1_path

    dimana label_xx_path berisi daftar posisi x,y dari kepala.
    """
    ...
    def __init__(self, root_path, crop_size, downsample_ratio=8, method="train"):
        ...
        if method not in ["train", "valid", "test"]:
            raise Exception("method harus salah satu dari: train, valid, test")

        # baca file list
        self.img_to_label = {}
        list_file = f"{method}.list"  # train.list, valid.list or test.list
        list_path = os.path.join(self.root_path, list_file)

        if not os.path.exists(list_path):
            raise FileNotFoundError(f"File {list_path} tidak ditemukan!")
        ...
        with open(list_path) as fin:
            for line in fin:
                ...
                if len(line) >= 2:
                    img_path = os.path.join(self.root_path, method, line[0].strip())
                    label_path = os.path.join(self.root_path, method, line[1].strip())
                    self.img_to_label[img_path] = label_path

        self.img_list = sorted(list(self.img_to_label.keys()))
        print("Jumlah gambar [{}]: {}".format(method, len(self.img_list)))
    ...
    def load_head_annotation(self, gt_path):
        """
        Load annotation dari file txt dengan format:
        x1 y1
        x2 y2
        ...
        """
        ...
        if not os.path.exists(gt_path):
            print(f"Warning: Label file {gt_path} tidak ditemukan")
            return np.array(annotations)

        try:
            with open(gt_path, "r") as annotation:
    ...
    def train_transform(self, img, keypoints):
        ...
        # resize gambar agar sesuai dengan crop size
        if st_size < self.c_size:
            rr = 1.0 * self.c_size / st_size
            wd = round(wd * rr)
            ht = round(ht * rr)
        ...
        assert st_size >= self.c_size, f"Image terlalu kecil: {wd}, {ht}"
        assert len(keypoints) >= 0
